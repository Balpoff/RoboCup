...
float boostR = 0, boostL = 0, boostB = 0;
int SSLEFT = 0, SSRIGHT = 0, SSBACK = 0;
bool CWB, CWL, CWR, inverted;
float Dkol = 1.45;
...
void SetVelPizhe(int targetLeft, int targetRight, int targetBack) {
  checkVel();
  if (targetLeft > 20) pwrL = 250;
  else if (targetLeft > 15) pwrL = 240;
  else if (targetLeft > 10) pwrL = 220;
  else if (targetLeft > 5) pwrL = 160;
  else pwrL = 40;

  if (targetRight > 20) pwrR = 250;
  else if (targetRight > 15) pwrR = 240;
  else if (targetRight > 10) pwrR = 220;
  else if (targetRight > 5) pwrR = 160;
  else pwrR = 40;

  if (targetBack > 20) pwrB = 250;
  else if (targetBack > 15) pwrB = 240;
  else if (targetBack > 10) pwrB = 220;
  else if (targetBack > 5) pwrB = 160;
  else pwrB = 40;

  if (targetRight == 0) {
    analogWrite(9, 0);
  }
  else if (targetRight > 25){
    if (vel1>=targetRight){
      analogWrite(9, 180);
      boostR = 0;
      }
      else{
        analogWrite(9, 250 + boostR);
        boostR += 2;
        }
    }
  else if (targetRight > 5) {
    if (vel1 >= targetRight) {
      analogWrite(9, 0);
      boostR = 0;
    }
    else {
      analogWrite(9, pwrR + boostR);
      boostR += 1;
    }
  }
  else {
    if (vel1 >= targetRight) {
      analogWrite(9, boostR);
      boostR = 0;
    }
    else {
      analogWrite(9, pwrR + boostR);
  ...
  if (targetLeft == 0) {
    ...
    if (vel1>=targetLeft){
      ...
      boostL = 0;
        ...
        analogWrite(3, 250 + boostL);
        boostL += 2;
        }
    }
  else if (targetLeft > 5) {
    if (vel2 >= targetLeft) {
      analogWrite(3, 0);
      boostL = 0;
    }
    else {
      analogWrite(3, pwrL + boostL);
      boostL += 1;
    }
  }
  else {
    if (vel2 >= targetLeft) {
      analogWrite(3, boostL);
      boostL = 0;
    }
    else {
      analogWrite(3, pwrL + boostL);
  ...
  else if (targetBack > 25){
    if (vel1>=targetBack){
      analogWrite(10, 180);
      boostB = 0;
      }
      else{
        analogWrite(10, 250 + boostB);
        boostB += 2;
        }
    }
  else if (targetBack > 5) {
    if (vel3 >= targetBack) {
      analogWrite(10, 0);
      boostB = 0;
    }
    else {
      analogWrite(10, pwrB + boostB);
      boostB += 1;
    }
  }
  else {
    if (vel3 >= targetBack) {
      analogWrite(10, boostB);
      boostB = 0;
    }
    else {
      analogWrite(10, pwrB + boostB);
  ...
  /*
          if (vel2>=targetLeft) {analogWrite(3,0); boostL = 0;}
          else {analogWrite(3,pwrL + boostL); boostL++;}
         if (vel3>=targetBack) {analogWrite(10,0); boostB = 0;}
          else {analogWrite(10,pwrB + boostB); boostB++;}*/
}


void SoSetVelPizhe(int targetLeft, int targetRight, int targetBack) {
  checkVel();/*
    if (targetLeft>20) pwrL = 250;
    else if (targetLeft>15) pwrL = 240;
    else if (targetLeft>10) pwrL = 220;
  ...
  if (boostB > 255) boostB = 255;
  if (boostB > 255) boostB = 255;
  if (boostB > 255) boostB = 255;
  if (boostB < 0) boostB = 0;
  if (boostB < 0) boostB = 0;
  if (boostB < 0) boostB = 0;

  if (boostR > 255) boostR = 255;
  if (boostR > 255) boostR = 255;
  if (boostR > 255) boostR = 255;
  if (boostR < 0) boostR = 0;
  if (boostR < 0) boostR = 0;
  if (boostR < 0) boostR = 0;

  if (boostL > 255) boostL = 255;
  if (boostL > 255) boostL = 255;
  if (boostL > 255) boostL = 255;
  if (boostL < 0) boostL = 0;
  if (boostL < 0) boostL = 0;
  if (boostL < 0) boostL = 0;
...
void sread() {
  ...
  while (Serial.available() > 0) {
    ...
    switch ((incomingByte & 192) >> 6) { //dviz
        ...
        if (!digitalRead(13)) flag = 0;
      case (0):
        {
          if (digitalRead(13)) {
            if (((incomingByte & 32) >> 5) == 1) CWLEFT(); else CCWLEFT();
          } else {
      ...
      case (1):
        {
          if (digitalRead(13)) {
            if (((incomingByte & 32) >> 5) == 1) CWRIGHT();
            else CCWRIGHT();
          }
          else
      ...
      case (2):
        {
          if (digitalRead(13)) {
            if (((incomingByte & 32) >> 5) == 1) CWBACK();
            else CCWBACK();
          }
          else
          {
            if (((incomingByte & 32) >> 5) == 1) CCWBACK();
            else CWBACK();
          }
...
void invert() {
  if (inverted == 0) {
    if (digitalRead(13) == 1 && CWL == 1) {
      CCWLEFT();
    }
    else CWLEFT();

    if (digitalRead(13) == 1 && CWR == 1) {
      CCWRIGHT();
    }
    else CWRIGHT();

    if (digitalRead(13) == 1 && CWB == 1) {
      CCWBACK();
    }
    else CWBACK();
    inverted = 1;
  }
  if (digitalRead(13) == 0) inverted = 0;
}
void loop() {
  ...
  sread();
  //invert();
  SetVelPizhe(SSLEFT, SSRIGHT, SSBACK);
  //moveEnc(10000, 255);
  //travel(10000, 25);
  //travelVel(10000, 25);
  //angle(180);
